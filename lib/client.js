...
/* ===== Max effort: animated gradient in the spirit of DSH "deep diving" (1.8s shimmer).
...
`;
    ...
    // The native trigger always shows the current model name (same catalog source we read),
    // so match on it: a bare aria-haspopup query could catch another plugin's menu button
    // living in the same row. When the name does not match, refuse to hide anything.
    function findNativeModelButton(trailing, modelName) {
      try {
        const btns = Array.from(trailing.querySelectorAll(SHIPPED_MODEL_SEL));
        if (modelName) {
          const hit = btns.find((b) => {
            const txt = ((b.getAttribute("aria-label") || "") + " " + (b.textContent || ""));
            return txt.indexOf(modelName) !== -1;
          });
          return hit || null;
        }
        return btns[0] || null;
    ...
    function findTrailing(card, sliderRoot, modelName) {
      let p = sliderRoot.parentElement;
      while (p && p !== card) {
        try {
          // Ignore our own pill: we are looking for the container holding the NATIVE selector.
          if (findNativeModelButton(p, modelName)) return p;
        } catch {}
        p = p.parentElement;
      }
      return null;
    }
    function findModelWrap(trailing, sliderRoot, modelName) {
      try {
        ...
        const btn = findNativeModelButton(trailing, modelName);
        let modelWrap = null;
        if (btn) {
          let n = btn;
          while (n && n.parentElement !== trailing) n = n.parentElement;
          if (n && n !== sliderWrap) modelWrap = n;
        }
    ...
    function EffortSliderInner(props) {
      ...
      React.useEffect(() => {
        ...
        const trailing = findTrailing(card, sliderRoot, currentModelName);
        const restores = [];
        let modelWrap = null;
        if (trailing) {
          trailing.classList.add("efs-trailing");
          restores.push(() => { try { trailing.classList.remove("efs-trailing"); } catch {} });
          const found = findModelWrap(trailing, sliderRoot, currentModelName);
      ...
      // NOTE: no early return before ALL hooks below. Returning null here on single-effort
      // models used to change the hook count between renders, which crashed React
      // ("fewer hooks than expected") and killed the entry until page reload.
      ...
      if (currentIndex === -1 && efforts.length > 0) { const fb = (reasoning && reasoning.defaultEffort !== undefined) ? reasoning.defaultEffort : (efforts[0] && efforts[0].id); for (let i = 0; i < efforts.length; i++) if (efforts[i].id === fb) { currentIndex = i; break; } if (currentIndex === -1) currentIndex = 0; }
      ...
      // Safe to hide now: every hook above has run unconditionally.
